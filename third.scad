...
VISUALIZE_LEGENDS = true; // Set to true to have the legends appear via %

// Width of a standard "key unit" (how much space a "key" takes up in total). Probably don't want to change this.
KEY_UNIT = 19; // Square that makes up the entire space of a key
...
KEY_ROTATION = [-45,0,0]; // An example of how you'd rotate a keycap on its side.  Make sure to zoom in on the bottom to make sure it's *actually* going to print flat! This should be the correct rotation for riskeycap profile.  For GEM use:
...
STEM_LOCATIONS = [ // Where to place stems/stabilizers
    [0,0,-0.2], // Dead center (don't comment this out when uncommenting below)
...
// DEBUGGING FEATURES
DEBUG = true; // Set this to true if you want poly_keycap() to spit out all it's variables in the console
